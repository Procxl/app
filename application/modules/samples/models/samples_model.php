...
class Samples_model extends BF_Model {
	...
	public function get_top_samples($top = 3){
		...
		$query = $this->db->select('samples.id, samples.quantity, product.name product_name, samples.date_received, vendors.name vendor_name, uom.name uom_name')
							->from('samples')
							->join('vendors', 'samples.vendor_id = vendors.id')
